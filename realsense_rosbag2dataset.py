...
for topic, msg, t in rosbag.Bag(sys.argv[1]).read_messages():
    ...
    if topic == '/device_0/sensor_0/Depth_0/image/data':
        ...
        cv2.normalize(image_depth, image_depth, 0, 255, cv2.NORM_MINMAX)
...
for timestamp, image_color, image_depth in tqdm(zip(
    ...
    cv2.imwrite(
        ...
        cv2.resize(
            ...
            (image_depth.shape[1],
             image_depth.shape[0])))
    cv2.imwrite('data/depth/depth{:012}.jpg'.format(t), image_depth)

    color_raw = o3d.io.read_image('data/color/color{:012}.jpg'.format(t))
    depth_raw = o3d.io.read_image('data/depth/depth{:012}.jpg'.format(t))
